				...
				if (mysqli_num_rows($ans) > 0)
				{
					while($b = mysqli_fetch_assoc($ans))
					{
						...
						if ($b["itemCategory"] === "Pizza" or $b["itemCategory"] === "Burger" )
						{
							...
							echo "<label for=\"toppings\">Tambahan</label>";
...
               ?> 
			   ...
			   <?php 
			    ...
			    if (mysqli_num_rows($result) > 0)
				{
					...
					echo "<div class=\"form-group\">";
					echo "<label for=\"size\">Ukuran</label>";
...
               ?> 
		  	...
		  	<div class="form-group">
 				<input type="submit" class="btn btn-lg btn-default button" id="signup-button" name="addItem" value="Tambah Ke keranjang">
			</div>
		  </form>
		</div>
	</div>
